...
-- Same code as Storage.server.lua but on client will work

local client = Talkie.Client(game.ReplicatedStorage.Services, "StorageService")
local client1 = Talkie.Client(game.ReplicatedStorage.Services, "StorageService")
--- < v1.24.0
-- server/MyCoolService.lua
local event0 = client:Event("CachedEvent")
-- Need to export this event if I want to use it in other scripts

-- server/MyAnotherCoolService.lua (create talkie, etc..)
local cachedEvent = client1:Event("CachedEvent") -- got error or overwrited the remote, nip nip nip
...
print("[C] isTheSame: ", isTheSame) -- -> true 
...
do
  ...
  print("[C] isTheSame2: ", event == event0)
